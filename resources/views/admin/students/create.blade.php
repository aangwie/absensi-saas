...
<div class="max-w-2xl">
    <div class="bg-white rounded-2xl border border-slate-200/60 shadow-sm p-6">
        <div class="mb-5 p-3 rounded-xl bg-amber-50 border border-amber-200 text-amber-700 text-xs">
            <strong>Info:</strong> Password default siswa adalah <strong>NPSN sekolah</strong>. Siswa login melalui API menggunakan NISN + Password.
        </div>
        <form method="POST" action="{{ route('admin.students.store') }}" class="space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="text" name="nisn" value="{{ old('nisn') }}" required class="w-full px-4 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 font-mono" placeholder="0012345678">
                ...
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" required class="w-full px-4 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" placeholder="Ahmad Fauzi">
                    @error('name')<p class="text-xs text-red-500 mt-1">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1.5">Kelas</label>
                    <input type="text" name="class" value="{{ old('class') }}" class="w-full px-4 py-2.5 rounded-xl border border-slate-300 text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500" placeholder="IX-A">
