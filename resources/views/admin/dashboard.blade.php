    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            @if(auth()->user()->isSuperAdmin())
                <div
                    class="relative overflow-hidden bg-white rounded-2xl border border-slate-200/60 p-5 shadow-sm hover:shadow-md transition-shadow">
                    <div class="relative z-10 flex items-center justify-between">
                        ...
                        <div
                            class="w-12 h-12 rounded-xl bg-gradient-to-br from-violet-500 to-purple-600 flex items-center justify-center shadow-lg shadow-violet-500/30">
            ...
            <div
                ...
                <div class="relative z-10 flex items-center justify-between">
                    ...
                    <div
                        class="w-12 h-12 rounded-xl bg-gradient-to-br from-amber-500 to-orange-600 flex items-center justify-center shadow-lg shadow-amber-500/30">
                        <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                ...
                <div class="relative z-10 flex items-center justify-between">
                    <div>
                        ...
                        <div class="flex items-center gap-2 mt-1">
                            <span
                                class="inline-flex items-center gap-1 text-[10px] font-semibold text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded-md">{{ $todayOnTime }}
                                tepat</span>
                            <span
                                class="inline-flex items-center gap-1 text-[10px] font-semibold text-red-600 bg-red-50 px-1.5 py-0.5 rounded-md">{{ $todayLate }}
                                telat</span>
                        </div>
                    </div>
                    <div
            ...
            <div class="lg:col-span-2 bg-white rounded-2xl border border-slate-200/60 p-6 shadow-sm">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div class="flex-1 flex items-center gap-1 h-7">
                                ...
                                @if($day['on_time'] == 0 && $day['late'] == 0)
                                    <div
                                        class="h-full bg-slate-100 rounded-lg flex items-center justify-center text-[10px] text-slate-400 px-2 w-full">
                                        Belum ada data
                                    </div>
                                @endif
                            </div>
                ...
                <div class="flex items-center gap-4 mt-4 pt-3 border-t border-slate-100">
                    <div class="flex items-center gap-1.5">
                        <div class="w-3 h-3 rounded bg-emerald-500"></div><span class="text-[10px] text-slate-500">Tepat
                            Waktu</span>
                    </div>
                    <div class="flex items-center gap-1.5">
                        <div class="w-3 h-3 rounded bg-red-500"></div><span
                            class="text-[10px] text-slate-500">Terlambat</span>
                    </div>
                </div>
            </div>
                ...
                <div class="space-y-3">
                    @forelse($recentAttendances as $att)
                        <div class="flex items-start gap-3 p-2 rounded-xl hover:bg-slate-50 transition-colors">
                            <div
                                class="w-8 h-8 rounded-lg {{ $att->status === 'on_time' ? 'bg-emerald-100 text-emerald-600' : 'bg-red-100 text-red-600' }} flex items-center justify-center shrink-0 mt-0.5">
                            ...
                            <span
                                class="inline-flex text-[10px] font-semibold px-2 py-0.5 rounded-full {{ $att->status === 'on_time' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700' }}">
